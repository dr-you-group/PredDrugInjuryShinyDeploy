...
server <- shiny::shinyServer(function(input, output, session) {
  ...
  output$summaryTable <- DT::renderDataTable(DT::datatable(summaryTable[filterIndex(),!colnames(summaryTable)%in%c('addExposureDaysToStart','addExposureDaysToEnd', 'plpResultLocation', 'plpResultLoad')],
                                             ...
                                             container = htmltools::withTags(table(
                                               ...
                                               thead(
                                                 ...
                                                 tr(apply(data.frame(colnames=c('Dev', 'Val', 'Target cohort','Outcome cohort', 'Model','Covariate setting',
                                                                                'TAR', 'AUC', 'AUPRC', 
  ...
  plpResult <- shiny::reactive({getPlpResult(result,validation,summaryTable, inputType, resultRow())})
  ...
  # prediction text
  #output$info <- shiny::renderUI(shiny::HTML(paste0(shiny::strong('Model: '), summaryTable[resultRow(),'Model'], ' with covariate setting id ',summaryTable[resultRow(),'covariateSettingId'] , '<br/>',
  #                                                  shiny::strong('Question:'), ' Within ', summaryTable[resultRow(),'T'],
  #                                        ' predict who will develop ',  summaryTable[resultRow(),'O'],
  #                                        ' during ',summaryTable[resultRow(),'TAR'], '<br/>',
  #                                        ' Developed in database: ', shiny::strong(summaryTable[resultRow(),'Dev']), ' and ',
  #                                        ' validated in database:  ', shiny::strong(summaryTable[resultRow(),'Val'])
  #                                 ))
  #)
  
  output$sideSettings  <- shiny::renderTable(t(data.frame(Development = as.character(summaryTable[resultRow(),'Dev']), 
                                                        Validation = as.character(summaryTable[resultRow(),'Val']),
                                                        Model = as.character(summaryTable[resultRow(),'Model']))), rownames = T, colnames = F)
  
  output$sideSettings2  <- shiny::renderTable(t(data.frame(T = paste0(substring(as.character(summaryTable[resultRow(),'T']),0,25),'...') , 
                                                           O = paste0(substring(as.character(summaryTable[resultRow(),'O']),0,25),'...')  )), 
  ...
  resultRow <- shiny::reactiveVal(value = 1)
  ...
  shiny::observeEvent(input$summaryTable_rows_selected,{
    ...
    resultRow(filterIndex()[input$summaryTable_rows_selected])
    shiny::updateSelectInput(session, "selectResult",
                           selected = myResultList[[resultRow()]]
                           )
  })
