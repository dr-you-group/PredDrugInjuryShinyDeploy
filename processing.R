...
getSummary  <- function(result,inputType,validation){
  ...
  colnames(sumTab) <- c(
    'Dev', 'Val', 'T', 'O','Model','covariateSetting', 
...
getSummaryFromObject <- function(result, analysisId = NULL){
  ...
  TAR <- 'cohort start + 1 - cohort start + 60'
  ...
  allRes <- data.frame(analysisId = ifelse(is.null(result$analysisId), ifelse(is.null(result$analysisRef$analysisId), 'None', result$analysisRef$analysisId), analysisId),
...
getPerformance <- function(analysisLocation){
  ...
  if(type == 'runPlp'){
    ...
    res$model$trainDetails$cdmDatabaseSchema <- name[[1]][length(name[[1]])-1]
